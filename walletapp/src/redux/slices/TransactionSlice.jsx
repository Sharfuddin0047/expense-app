  ...
  async ({ walletId, transaction }, { rejectWithValue, dispatch }) => {
    try {
      const result = await axios.post(
        `${API_BASE_URL}/transaction/addTransaction/${walletId}`,
        transaction
      );
...
export const fetchOneTransaction = createAsyncThunk(
  ...
  async ({ walletId, id }) => {
    const response = await axios.get(
      `${API_BASE_URL}/transaction/getTransactionById/${walletId}/${id}`
    );
...
export const fetchAllTransaction = createAsyncThunk(
  ...
  async (walletId, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/transaction/getAllTransaction/${walletId}`
...
export const updateTransactionById = createAsyncThunk(
  ...
  async ({walletId, id, transaction}) => {
    const response = await axios.put(
      `${API_BASE_URL}/transaction/updateTransaction/${walletId}/${id}`,
      transaction
    );
    return response.data;
  }
...
export const deleteTransaction = createAsyncThunk(
  ...
  async ({ walletId, transId }, { rejectWithValue }) => {
    try {
      await axios.delete(
        `${API_BASE_URL}/transaction/deleteTransactionById/${walletId}/${transId}`
      );
      return transId;
    } catch (error) {
